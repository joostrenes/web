        ...
        )
{
    ...
    group_scalar_get32((group_scalar*)&rx, sk+32);
    ladder_base(&R, (group_scalar*)&rx);
        ...
        )
{
    ...
    *smlen = mlen+64;
    ...
    hash(sm, sm, mlen+64);
    group_scalar_get64((group_scalar*)&R.X, sm);
    group_scalar_get32((group_scalar*)&R.Z, sk+32);

    group_scalar_set_pos((group_scalar*)&R.X);
    group_scalar_mul((group_scalar*)&R.Z, (group_scalar*)&R.X, (group_scalar*)&R.Z);
    group_scalar_sub((group_scalar*)&R.Z, &r, (group_scalar*)&R.Z);

    fe25519_pack(sm, &rx);
    group_scalar_pack(sm+32, (group_scalar*)&R.Z);
        ...
        )
{
    ...
    group_scalar_get32((group_scalar*)&bZZ, sm+32);
    for(i=0;i<smlen-64;i++) { m[i] = sm[64+i]; }
    mlen = smlen-64;

    for(i=0;i<32;i++) { sm[32+i] = pk[i]; }
    hash(sm, sm, mlen+64);
    group_scalar_get64((group_scalar*)&bXZ, sm);

    fe25519_unpack(&bXX, pk);
    decompress(&sP, &bXX);
    ladder(&hQ, &sP, &bXX, (group_scalar*)&bXZ);
    ladder_base(&sP, (group_scalar*)&bZZ);

    bValues(&bZZ, &bXZ, &bXX, &sP, &hQ);
    return check(&bZZ, &bXZ, &bXX, &rx, &sP);
}

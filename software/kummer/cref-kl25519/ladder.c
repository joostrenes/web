...
void xDBLADD(ecp *xp, ecp *xq, const fe25519 *xd)
{
    ...
    fe25519_mulconst(&xp->X, &xq->X, 0x4D, 0x00);
    fe25519_mulconst(&xp->Z, &xq->Z, 0x52, 0x00);

    fe25519_mulconst(&b0, &b0, 0x4D, 0x00);
    fe25519_mulconst(&b1, &b1, 0x52, 0x00);
    ...
    fe25519_mulconst(&xp->X, &b0, 0x05, 0x00);
    fe25519_mulconst(&xp->Z, &b1, 0x9F, 0x00);
...
void ladder(ecp *r, ecp *xp, const fe25519 *xpw, const group_scalar *n)
{
    ...
    fe25519_setone(&r->X);
    fe25519_setone(&r->Z);
    fe25519_mulconst(&r->X, &r->X, 0x9F, 0x00);
    fe25519_mulconst(&r->Z, &r->Z, 0x05, 0x00);
        ...
        const ecp *xp, const ecp *xq)
{
    ...
    fe25519_mulconst(bZZ, bZZ, 0x1B, 0x03);
    ...
    fe25519_mulconst(bXX, bXX, 0x1B, 0x03);
    fe25519_add(bXZ, &b1, &b2);

    fe25519_mul(bXZ, bXZ, &b0);
    fe25519_mulconst(bXZ, bXZ, 0x1B, 0x03);
    fe25519_mul(&b0, &b1, &b2);
    fe25519_add(&b0, &b0, &b0);
    fe25519_add(&b0, &b0, &b0);
    fe25519_add(&b1, &b0, &b0);
    fe25519_mulconst(&b1, &b1, 0x29, 0x17);
    fe25519_mulconst(&b0, &b0, 0x1B, 0x03);
        ...
        const ecp *rx)
{
    ...
    fe25519_square(&b0, &rx->X);
    fe25519_mul(&b0, &b0, bXX);
    fe25519_mul(&b1, &rx->X, &rx->Z);
    fe25519_mul(&b1, &b1, bXZ);
    fe25519_sub(&b0, &b0, &b1);
    fe25519_square(&b1, &rx->Z);
...
void kl_to_mont(ecp *r)
{
    fe25519 b0,b1;
    
    fe25519_mulconst(&b0, &r->X, 0x9F, 0x00);
    fe25519_mulconst(&b1, &r->X, 0x05, 0x00);
    fe25519_mulconst(&r->X, &r->Z, 0x05, 0x00);
    fe25519_sub(&r->X, &r->X, &b0);
    fe25519_mulconst(&r->Z, &r->Z, 0x9F, 0x00);
